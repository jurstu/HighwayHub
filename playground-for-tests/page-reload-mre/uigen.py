...
class UiGen:
    ...
    def loadRadars(self):
        ...
        try:
            with open("canard_detailed_data.json", "r") as f:
                data = json.load(f)
        ...
        detectionRadius = 500 #self.ic.getValue("RadarWatcherObject").detectionEntryRadius
    ...
    def handleMouseMove(self, e: events.GenericEventArguments):
        ...
        for m in self.justMaps:
            if(m._client() != None):
                m.marker(latlng=(lat, lon), options={'rotationAngle': 90, "rotationOrigin": "center center"})
